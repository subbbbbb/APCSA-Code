...
public class JavaSubset {
	public static void main(String[] args) {
		...
		// escape sequences
		System.out.println("\""); // '\' is the escape character, the whole thing is an escape sequence
		...
		System.out.println(twoD[0].length); // this prints out the number of columns (up down)
			//printing out a full 2D array
		for(int row = 0; row< twoDD.length; row++) {
			for(int col = 0; col<twoDD[row].length; col++) {
				System.out.print(twoDD[row][col] + " ");
			}
			System.out.println();
		}
		...
		List<Integer> arrL = new ArrayList<Integer>(); // an arraylist is-a list, but a list isn't necessarily an
														// arraylist
		...
		// for and while loops
		int xWhile = 4;
		for(int i = 0; i<xWhile; i++) {
			System.out.println("This is printed for each for loop.");
		}
		
		while(xWhile < 7) {
			System.out.println("This is printed for each while loop.");
			xWhile++;
		}
