...
public class DailySchedule
{
  ...
  {
    if (emergency) // if it is an emergency
    {
      clearConflicts(appt);
      apptList.add(appt);
      return true;
    ...
    for (int i = 0; i < apptList.size(); i++)
      if (((Appointment)apptList.get(i)).conflictsWith(appt)) // if it does conflict with
        return false;

    apptList.add(appt); // if it doesn't conflictWith and it is not an emergency
    return true;
  }
